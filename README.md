...
* src\actions - действия для flux
* src\reducers - редьюсеры для flux
* src\store - работа с хранилищем flux
* src\containers - "умные" компоненты react связанные с хранилищем flux
* src\interfaces - папка с интерфейсами для typescript
...
Была реализована библиотека [shrilux](https://github.com/KingManiya/shrilux). Библиотека покрыта тестами, подключен travis-ci и coveralls для отображения покрытия кода тестами.

#### Использование flux
Для примера работы с библиотекой было реализовано 4 действия:
- Получение данных событий
- Получение данных камер
- Сохранение настроек контрасности камеры
- Сохранение настроек яркости камеры

Настройки камер хранятся на сервере и восстанавливаются на клиенте, после перезагрузки страницы (при получении данных камер).

Было решено не подключать flux к работе с вкладками, так как для переключения уже используется react-router, и отказ от него был бы фактичски откатом функционала работы с url и навигацией браузера.

Поток:
1. Приложение или пользователь вызывают действия
2. Действия вызывают функцию dispatch хранилища передавая объект с типом
3. Хранилищи реагирует и в reducer распределяет данные по состоянию
4. Контейнеры по подписке узнают об озменинии состояния и устанавливают себе новое состояние на основании состояния хранилища. Дочернчие компоненты получают данные через props.
